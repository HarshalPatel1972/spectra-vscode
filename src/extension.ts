...
export function activate(context: vscode.ExtensionContext) {
    ...
    const hoverProvider = vscode.languages.registerHoverProvider(
        ['go', 'python', 'java', 'javascript', 'typescript', 'rust', 'c', 'cpp'],
        {
            provideHover(document, position, token) {
                const uri = document.uri.toString();
                const line = position.line;
                
                const finding = latestFindings.find(f => 
                    vscode.Uri.file(f.file_path).toString() === uri && 
...
function processSpectraOutput(jsonStr: string) {
    try {
        ...
        if (result.aggregate_qrs !== undefined) {
            ...
            if (result.aggregate_qrs >= 80) statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.errorBackground');
            else if (result.aggregate_qrs >= 40) statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
